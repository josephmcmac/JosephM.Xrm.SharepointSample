...
namespace JosephM.Xrm.SharepointSample.Plugins.Services.SharePoint
{
    ...
    public class SharepointService
    {
        ...
        public string UploadDocument(string documentFolderUrl, string fileName, string fileContent, string prefix = null, string postfixUnique = null)
        {
            ...
            var serialiser = new DataContractJsonSerializer(typeof(RootDocument));
            var deser = serialiser.ReadObject(GenerateStreamFromString(response));
            var deserAs = deser as RootDocument;

            return string.Format("{0}/{1}/{2}", SharepointSiteUri, documentFolderUrl, sharepointFileName);
        }

        public string GetEditWordDocumentUrl(Guid uniqueId, string fileName)
        {
            var url = string.Format("{0}/:w:/r/{1}/_layouts/15/Doc.aspx?sourcedoc={2}&file={3}&action=edit&mobileredirect=true", SharepointsiteRoot, SharepointSiteUriLocalPathNoLeadingSlash, uniqueId.ToString("B"), fileName);
            return url;
        }
        ...
        public string ReplaceInvalidChars(string fileName)
        {
            ...
            fileName = new string(fileName.Select(c => char.IsLetterOrDigit(c) || c == ' ' || c == '.' ? c : '_').ToArray());

            return fileName;
        }

        private string GenerateDocumentFolderName(Guid id, string name)
        {
            if (name == null)
                name = string.Empty;
            ...
            var truncateName = name.Left((maxLength - guidPart.Length) - 1);
            return truncateName + "_" + guidPart;
        }

        public string GetOrCreateDocumentFolderRelativeUrl(string type, Guid id, string recordName)
        {
            var getUrl = GetDocumentFolderRelativeUrl(type, id);
            if (getUrl != null)
                return getUrl;

            //query crm document folders for the record type
            var rootFolderQuery = XrmService.BuildQuery(Entities.sharepointdocumentlocation, null, new[] { new ConditionExpression(Fields.sharepointdocumentlocation_.relativeurl, ConditionOperator.Equal, type) }, null);
            AddParentNodeLinks(rootFolderQuery);
            var matchingFolders = XrmService.RetrieveAll(rootFolderQuery);
            if (!matchingFolders.Any())
            {
                //if doesn't use native integration (no crm folder matched)
                //then check if the folder has been created in sharepoint an dif not create a new folder in sharepoint for this type (non-native integration folder)
            ...
            var entityFolderName = GenerateDocumentFolderName(id, recordName);
            ...
            if (matchingFolders.Any())
            {
                ...
                sharepointLocation.SetField(Fields.sharepointdocumentlocation_.name, entityFolderName);
            ...
            return typeFolderName + "/" + entityFolderName;
        }
        ...
        {
            [DataMember(Name = "d")]
            public SharepointDocument Document { get; set; }

            //[DataContract]
            //public class d
            //{
            //    [DataMember(Name = "results")]
            //    public SharepointDocument results { get; set; }
            //}
        }
        ...
        public IEnumerable<SharepointDocument> GetDocuments(string folderRelativeUrl, string startsWith = null, int top = -1)
        {
            ...
            var restQuery = string.Format("/_api/web/getfolderbyserverrelativeurl('{0}')/files?$select=Name,UniqueId{1}", serverRelativeFolder, startsWithArg);
            var httpVerb = "GET";
            string data = null;
        ...
        public string GetDocumentFolderRelativeUrl(string regardingType, Guid regardingObjectId)
        {
            ...
            string documentFolderUrl = GetPathIncludingParentNodes(firstResult);
        ...
        private static string GetPathIncludingParentNodes(Entity firstResult)
        {
            ...
            if (firstResult.GetFieldValue("LINK3." + Fields.sharepointdocumentlocation_.relativeurl) != null)
                url.Append(firstResult.GetFieldValue("LINK3." + Fields.sharepointdocumentlocation_.relativeurl) + "/");
            if (firstResult.GetFieldValue("LINK2." + Fields.sharepointdocumentlocation_.relativeurl) != null)
                url.Append(firstResult.GetFieldValue("LINK2." + Fields.sharepointdocumentlocation_.relativeurl) + "/");
            if (firstResult.GetFieldValue("LINK1." + Fields.sharepointdocumentlocation_.relativeurl) != null)
                url.Append(firstResult.GetFieldValue("LINK1." + Fields.sharepointdocumentlocation_.relativeurl) + "/");
        ...
        private Entity SharepointSite
        {
            get
            {
                if (_sharepointSite == null)
                {
                    ...
                    if (_sharepointSite == null)
                        throw new NullReferenceException(string.Format("Error could not find the {0} record", XrmService.GetEntityLabel(Entities.sharepointsite)));
                }
        ...
        private string SharepointSiteUriLocalPathNoLeadingSlash
        {
            get
            {
                return SharepointSiteUriLocalPath.StartsWith("/") ? SharepointSiteUriLocalPath.Substring(1) : SharepointSiteUriLocalPath;
            }
        }

        private string SharepointsiteRoot
        {
            get
            {
                return SharepointSiteUri.Substring(0, SharepointSiteUri.Length - SharepointSiteUriLocalPath.Length);
            }
        }
